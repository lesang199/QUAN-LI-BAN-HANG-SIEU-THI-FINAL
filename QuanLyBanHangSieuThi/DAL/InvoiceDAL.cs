...
namespace QuanLyBanHangSieuThi.DAL
{
    public class InvoiceDAL
    {
        ...
        private string _dataFile = Path.Combine(Global.DATA_PATH, nameof(Invoice) + ".json");
        ...
        // Phương thức nạp danh sách Invoice từ file JSON
        public List<Invoice> Load()
        {
            if (File.Exists(_dataFile))
            {
                try
                {
                    ...
                    return JsonSerializer.Deserialize<List<Invoice>>(jsonString, options);
                }
                catch (Exception ex)
                {
            ...
            return new List<Invoice>(); // Trả về danh sách trống nếu không có file
        }

        // Phương thức lưu danh sách Invoice vào file JSON
        public void Save(List<Invoice> Invoices)
        {
            try
            {
                ...
                var jsonString = JsonSerializer.Serialize(Invoices, options);
